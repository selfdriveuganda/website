...
export const BlogPostCard = ({ blog }: { blog: SanityDocument }) => {
	...
	const handleNavigateToBlog = (blogSlug: string) => {
		navigate({
			to: "/blogs/$blogSlug",
			params: { blogSlug },
		});
	...
	return (
		<div
			className="group relative block h-[350px] w-full max-w-full cursor-pointer overflow-hidden rounded-xl sm:h-[400px] sm:max-w-[358px] md:h-[460px]"
			onClick={() => handleNavigateToBlog(blog.slug.current)}
			onKeyDown={(e) => {
				if (e.key === "Enter" || e.key === " ") {
					e.preventDefault();
					handleNavigateToBlog(blog.slug.current);
				}
			}}
			role="button"
			tabIndex={0}
